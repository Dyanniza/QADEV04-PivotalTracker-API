...
var getProject = function (id, token, callback) {
	...
	if (id == -1) {
		...
		request
			...
			.end(function(err, res){
				if (err) {
					console.log('Error Get Project'+JSON.stringify(err));
				} else{
					console.log(' Get Project '+JSON.stringify(res.body));
				};
				callback(res);
...
var createProject = function(prj, token, callback) {
    ...
    request
        ...
        .end(function(err, res) {
            ...
            if (err) {
                console.log('Error Create Project' + JSON.stringify(err));
            } else {
...
var editProject = function(argument, id, token, callback) {
    ...
    request
        ...
        .end(function(err, res) {
            ...
            if (err) {
                console.log('Error Update Project' + JSON.stringify(err));
            } else {
